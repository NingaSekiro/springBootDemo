...
public class TransactionAop {
    ...
    @Around(value = "@annotation(mainTransaction) && args(threadCount,..)")
    public void mainIntercept(ProceedingJoinPoint joinPoint, MainTransaction mainTransaction,Integer threadCount) throws Throwable {
        //当前线程名称
...
//        Expression expression = parser.parseExpression(mainTransaction.value());
//        EvaluationContext context = new StandardEvaluationContext();
//        for (int len = 0; len < params.length; len++) {
//            context.setVariable(params[len], args[len]);
//        }
//        Integer count = expression.getValue(context,Integer.class);
        CountDownLatch sonDownLatch = new CountDownLatch(threadCount);//@MainTransaction注解中的参数, 为子线程的数量
    ...
    @Around("@annotation(com.example.springdemo.demos.web.anno.SonTransaction)")
